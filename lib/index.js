...
export default function validate(schema = {}, formData = {}) {
  ...
  function processValidation(key) {
    schema[key].some((value) => {
      let result = null;
      const data = getDeepValue(formData, key);
      ...
      if (typeof value === 'string' && value in rules) {
        ...
        result = rules[value].isValid(data, rules[value].error, option);
      }
      ...
      if (typeof value === 'object' && value !== null) {
        ...
        if (typeof value.isValid === 'function') {
          result = value.isValid(data, value.error, option);
        } else if (value.rule in rules) {
          result = rules[value.rule].isValid(data, value.error, option);
        } else {
