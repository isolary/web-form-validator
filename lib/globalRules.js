...
export default {
  ...
  required: {
    isValid: (input = '', error = 'The input field is requried') => (
      input.toString().trim().length > 0 ? true : error
    ),
  },
  phone: {
  ...
  minLength: {
    isValid: (input = '', error, length = 0) => (
      input.length >= length ? true : (error || `The input length should be at least ${length} characters`)
    ),
  },
  maxLength: {
    isValid: (input = '', error, length = 0) => (
     input.length <= length ? true : (error || `The input length should be ${length} characters maximum`)
    ),
  },
};
