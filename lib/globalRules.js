...
export default {
  ...
  isEqual: {
    isValid: (input = '', error, match = '') => (
      isEqual(input, match) ? true : (error || 'The values are not equal')
    ),
  },
  minLength: {
    isValid: (input = '', error, length) => (
      length === undefined || input.length >= length ? true : (error || `The input length should be at least ${length} characters`)
    ),
  },
  maxLength: {
    isValid: (input = '', error, length) => (
     length === undefined || input.length <= length ? true : (error || `The input length should be ${length} characters maximum`)
    ),
  },
};
